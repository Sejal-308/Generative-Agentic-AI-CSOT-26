...
def trim_history(messages: list[dict], max_turns: int) -> list[dict]:
    ...
    if len(messages) > max_allowed_length:
        ...
        messages = system_prompt + newest_messages
...
class ChatApp(App):
    ...
    async def _get_response(self) -> None:
        ...
        try:
            ...
            self.messages.append({"role": "assisstant", "content":reply})
            self.call_from_thread(log.write, f"[bold blue]Agent:[/bold blue] {reply}")
        except Exception as Error:
            self.call_from_thread(log.write, f"[bold red]Error:[/bold red] {Error}")
    ...
    def action_clear_history(self) -> None:
        ...
        self.query_one("#log", RichLog).clear()
        self.query_one("#log", RichLog).write("[dim]History cleared.[/dim]")
