...
def dispatch(tool_call) -> str:
    ...
    try:
        j_args=json.loads(tool_call.function.arguments)
        
        if name in TOOL_REGISTRY:
            funct=TOOL_REGISTRY[name]
            return json.dumps(funct(**j_args))
        else:
...
def run_agent(user_message: str) -> str:
    ...
    for i in range(MAX_ITERATIONS):
        ...
        if finish_reason=="stop":
            ...
            print(f'Unexpected finish reason: {finish_reason}', file=sys.stderr)
